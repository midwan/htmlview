...
because the only decoder in active use (`DecoderThread` in
`ImageManager.cpp`) passes `TransparencySINGLE`/`NONE` and never
`TransparencyALPHA` to `RenderEngine::AllocateFrame`. If a future
decoder starts populating `AlphaMask`, that path needs to either
skip alpha-blending when scaling is in play or scale the alpha
plane through the `BitmapScaler` API.
